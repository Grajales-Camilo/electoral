...
const MUNICIPIOS_AMVA = [
    'MEDELLÍN', 'BELLO', 'ITAGÜÍ', 'ENVIGADO', 'SABANETA', 
    'LA ESTRELLA', 'CALDAS', 'COPACABANA', 'GIRARDOTA', 'BARBOSA'
];

// --- COMPONENTE PRINCIPAL ---
const MapaVoluntariosAMVA: React.FC<MapaVoluntariosAMVAProps> = ({ voluntarios }) => {
    ...
    useEffect(() => {
        const fetchGeoJson = async () => {
            ...
            try {
                ...
                if (!response.ok) {
                    throw new Error(`Error al cargar el GeoJSON del AMVA: ${response.statusText}`);
                }
    ...
    if (loading) return <p className="text-center text-slate-600">Cargando mapa del AMVA...</p>;
    if (error) return <p className="text-center text-red-500">Error al cargar el mapa: {error}</p>;

    return (
        <div className="h-[600px] w-full bg-white rounded-lg shadow-md p-4 relative mt-8">
...
export default MapaVoluntariosAMVA;
